...
function evaluateHand(cards) {
  ...
  if (isRoyalFlush) {
    return { rank: 9, name: HAND_NAMES[9], score: 9000000, cards }
  }

  if (flush && straight) {
    const highCard = RANK_VALUES[straight[0].rank]
    const score = 8000000 + highCard * 100
    return { rank: 8, name: HAND_NAMES[8], score, cards: straight }
  }

  if (groupSizes[0] === 4) {
    const quadRank = Object.entries(groups).find(([, g]) => g.length === 4)[0]
    const kicker = sorted.find(c => c.rank !== quadRank)
    const score = 7000000 + RANK_VALUES[quadRank] * 100 + (kicker ? RANK_VALUES[kicker.rank] : 0)
    const quadCards = [...groups[quadRank], kicker].filter(Boolean)
    return { rank: 7, name: HAND_NAMES[7], score, cards: quadCards }
  }
  ...
  if (groupSizes[0] === 3 && groupSizes[1] === 2) {
    ...
    const score = 6000000 + RANK_VALUES[trips] * 100 + RANK_VALUES[pair] * 10
    return { rank: 6, name: HAND_NAMES[6], score, cards }
  }
  ...
  if (straight) {
    const highCard = RANK_VALUES[straight[0].rank]
    const score = 4000000 + highCard * 100
    return { rank: 4, name: HAND_NAMES[4], score, cards: straight }
  }

  if (groupSizes[0] === 3) {
    const trips = Object.entries(groups).find(([, g]) => g.length === 3)[0]
    const kickers = sorted.filter(c => c.rank !== trips).slice(0, 2)
    const allCards = [...groups[trips], ...kickers]
    let score = 3000000 + RANK_VALUES[trips] * 100
    for (let i = 0; i < kickers.length; i++) {
      score += RANK_VALUES[kickers[i].rank] * Math.pow(15, 1 - i)
    }
    return { rank: 3, name: HAND_NAMES[3], score, cards: allCards }
  }
  ...
  if (groupSizes[0] === 2 && groupSizes[1] === 2) {
    ...
    const cardsUsed = [...groups[pairs[0]], ...groups[pairs[1]], kicker].filter(Boolean)
    return { rank: 2, name: HAND_NAMES[2], score, cards: cardsUsed }
  }

  if (groupSizes[0] === 2) {
    const pairRank = Object.entries(groups).find(([, g]) => g.length === 2)[0]
    const kickers = sorted.filter(c => c.rank !== pairRank).slice(0, 3)
    const cardsUsed = [...groups[pairRank], ...kickers]
    let score = 1000000 + RANK_VALUES[pairRank] * 100
    for (let i = 0; i < kickers.length; i++) {
      score += RANK_VALUES[kickers[i].rank] * Math.pow(15, 2 - i)
    }
    return { rank: 1, name: HAND_NAMES[1], score, cards: cardsUsed }
  }
...
export function determineWinner(players, communityCards) {
  ...
  return {
    ...
    allHands: players.map(p => {
      ...
      return { playerId: p.id, hand: r.hand }
    }),
  }
}
